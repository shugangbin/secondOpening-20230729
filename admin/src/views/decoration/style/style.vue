    ...
    <el-card shadow="never" class="!border-none px-[20px]">
        <div>
            ...
            <div class="mt-[20px] flex">
                <div class="py-[18px] px-[20px] flex items-center text-[14px] shadow rounded-lg mr-[20px]"
                    :class="{ isSelect: isSelect == item.id, isnSelect: isSelect != item.id }"
                    v-for="(item, index) in themeColors" :key="index" @click="selectThemeColor(item)">
                    <div class="rounded-full h-[34px] w-[34px]"
                        :style="`background: linear-gradient(to right, ${item.color1}, ${item.color2})`"></div>
        ...
        <div>
            ...
            <el-form label-width="140px" class="mt-[20px]">
                    ...
                    <div>
                        ...
                        <div>
                            ...
                            <el-popover placement="right" width="auto" trigger="hover">
                                ...
                                <template #default>
                                    <img src="@/assets/images/task_page.png" class="w-[200px] h-[230px]" />
                                </template>
                            </el-popover>
                        </div>
                    </div>
                ...
                <el-form-item label="导航顶部背景颜色">
                    <div>
                        <color-picker :defaultColor="navigationBarDefault"
                            v-model="formData.navigationBarColor"></color-picker>
                        <div>
                ...
                <el-form-item label="自定义主题颜色" v-if="isSelect == 5">
                    <div>
                        <color-picker v-model="formData.themeColor1" default-color="#F8F8F8"></color-picker>
                        <color-picker v-model="formData.themeColor2" default-color="#F8F8F8" class="mt-2"></color-picker>
                        <div>
...
//保存数据
const setData = async () => {
    await setDecoratePages({ id: 4, type: 4, data: JSON.stringify(formData.value) })
    getData()
}
