...
class UserLogic extends BaseLogic
{
    ...
    public static function detail(int $userId): array
    {
        $user = User::where(['id' => $userId])
            ...
            ->append(['channel_desc','inviter_name','invite_num'])
            ->findOrEmpty()
            ->toArray();

        //会员
        $user['member_desc'] = '未开通';
        $user['member_end_time_desc'] = '';
        $user['is_member'] = 0;
        $user['member_package_id'] = 0;
        if (isset($user['member_perpetual']) && $user['member_perpetual']) {
            $user['member_desc'] = '已开通';
            $user['member_end_time_desc'] = '永久';
            $user['is_member'] = 1;
        } elseif ($user['member_end_time']) {
            $user['member_desc'] = '已开通';
            $user['member_end_time_desc'] = date('Y-m-d H:i:s',$user['member_end_time']);
            $user['is_member'] = 1;
        }
        $user_member = UserMember::field('is_perpetual,package_name,member_end_time,package_info')->json(['package_info'],true)->where(['user_id'=>$userId,'refund_status'=>PayEnum::REFUND_NOT])->order(['is_perpetual'=>'desc','add_member_time'=>'desc','id'=>'desc'])->findOrEmpty()->toArray();
        if (!empty($user_member) && (!empty($user['member_end_time']) || $user['member_perpetual'])) {
            $user['member_desc'] = $user_member['package_name'];
            $user['member_package_id'] = $user_member['package_info']['id'];
        }
