...
class UserLogic extends BaseLogic
{
    ...
    public static function center(array $userInfo): array
    {
        ...
        if (isset($user['member_perpetual']) && $user['member_perpetual']) {
            $user['is_member'] = 1;
            $user['member_end_time'] = '永久';
        }
        $user_member = UserMember::field('is_perpetual,package_name,member_end_time')->where(['user_id'=>$userInfo['user_id'],'refund_status'=>PayEnum::REFUND_NOT])->order(['is_perpetual'=>'desc','add_member_time'=>'desc','id'=>'desc'])->findOrEmpty()->toArray();
        if (!empty($user_member) && (!empty($user['member_end_time']) || $user['member_perpetual'])) {
            $user['member_package_name'] = $user_member['package_name'];
        }
